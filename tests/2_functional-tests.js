...
function deleteThread(id, password){  
  chai.request(server)
    .delete(THREAD_URL)
...
/*
  suite('API ROUTING FOR /api/threads/:board', function() {
              ...
              chai.request(server)
                ...
                .end((err, res) => {
                  hErr(err, ()=> {
                    ...
                    // clean up the thread
                    deleteThread(thread.id, thread.delete_password);
    ...
    // need to slim it down? it times out a LOT
      ...
      test('DELETE with Incorrect Password', (done)=>{
        createThread((err, thread)=>{
          ...
          chai.request(server)
            ...
            .end((err, res)=>{
              // "DELETE with Incorrect Password" is the one situation where there should be an error
  ...
  */
  suite('API ROUTING FOR /api/replies/:board', function() {
                  ...
                  .end((err, res) => {
                    hErr(err, ()=> {
                      ...
                      // test the reply's created_on & thread's bumped_on Date; they should be:
                        // identical
                      assert.equal(res.body.replies[0].created_on, res.body.bumped_on, "Reply Created & Thread Bumped Dates are Identical");
                        // +/- 1 minute of now
                      assert.approximately(
    ...
    suite('DELETE', function() {
      ...
      test("Cleanup DB", (done)=>{
        deleteThread(repliesThread.id, repliesThread.delete_password);
